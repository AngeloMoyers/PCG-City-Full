...
public class PolygonDrawer : MonoBehaviour
{
	...
	public float m_extrudeDistance = 1f;
    ...
    public void Init()
    {
        ...
        if (mCollider == null)
        {
            mCollider = gameObject.GetOrAddComponent<MeshCollider>();
        }
    }
    ...
    public void Draw()
    {
		if (Time.time - startTime > 3.0f)
        {
			if (deactivateAfterTime)
            {
				...
				gameObject.GetComponent<DistrictPolygonBehavior>().SetActive(false);
			}
        }
        ...
        //extrude mesh
		if (m_extrudeDistance > 0.0f)
        {
			Mesh extrudedMesh = new Mesh();
			Matrix4x4[] extMatrix = { Matrix4x4.identity, Matrix4x4.Translate(m_extrudeDistance * Vector3.up) };

			MeshExtrusion.ExtrudeMesh(mesh, extrudedMesh, extMatrix, true);
			mesh = extrudedMesh;
		}
